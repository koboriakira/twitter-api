...
async def get_tweet(
    ...
    try:
        ...
        tweets = await use_case.execute(
            ...
            start_datetime=datetime.fromisoformat(start) if start is not None else jst_today_datetime() - timedelta(days=1),
            end_datetime=datetime.fromisoformat(end) if end is not None else jst_today_datetime(),
        )
